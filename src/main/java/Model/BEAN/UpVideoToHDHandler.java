...
public class UpVideoToHDHandler implements Runnable {
	...
	public UpVideoToHDHandler(String userID, int pID) {
		...
		this.userID = userID;
		this.pID = pID;
		inFilePath = rootFolder + File.separator + userID + File.separator + pID + File.separator + "output"
	...
	public void run() {
		...
		try {
			...
			if (exitCode == 0) {
				...
				file = new File(
						rootFolder + File.separator + userID + File.separator + pID + File.separator + "upload");
