...
public class MergeVideoHandler implements Runnable {
	private static final String ffmpegPath = "D:\\ffmpeg\\ffmpeg-n7.1-latest-win64-gpl-7.1\\bin\\ffmpeg.exe";
	...
	public MergeVideoHandler(String userID, int pID) {
		...
		File outFolder = new File(
				rootFolder + File.separator + userID + File.separator + pID + File.separator + "output");
		if (!outFolder.exists()) {
			outFolder.mkdirs();
		}
		outFilePath = rootFolder + File.separator + userID + File.separator + pID + File.separator + "output"
	...
	public void run() {
		...
		try {
			...
			ProcessBuilder processBuilder = new ProcessBuilder(
					...
					// VIDEO FILTERS – MERGE & FIX ORIENTATION + FPS
					"-vf",
